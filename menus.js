...
gameState.createColonyMenu = function () {
    ...
    this.colonyMenu.open = function () {
        ...
        this.repairButton.text.setText('repair ' + (pc.maxHp - pc.currentHp) * COLONY_REPAIR_COST_PER_HP + 'c');
...
gameState.colonyRepairShip = function () {
    ...
    if (pc.credits >= (pc.maxHp - pc.currentHp) * COLONY_REPAIR_COST_PER_HP) {
        pc.credits -= (pc.maxHp - pc.currentHp) * COLONY_REPAIR_COST_PER_HP;
        pc.repairDamage(1000);
        this.colonyMenu.open();
    } else if (pc.credits >= COLONY_REPAIR_COST_PER_HP) {
		pc.repairDamage(Math.floor(pc.credits / COLONY_REPAIR_COST_PER_HP));
		pc.credits -= Math.floor(pc.credits / COLONY_REPAIR_COST_PER_HP) * COLONY_REPAIR_COST_PER_HP;
		this.colonyMenu.open();
	}
};
